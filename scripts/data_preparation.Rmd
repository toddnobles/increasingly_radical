...
The CES as citylength in years for 2010, 2014, 2016, 2018
It has residency at current address for 2010, 2012, 2014, 2016, 2020. 
...
residency <- bind_rows(residency_2010, residency_2012,  residency_2014, residency_2016,  residency_2018,  residency_2020) 

gg_miss_var(residency, facet = year, show_pct = TRUE)
```


```{r}
# creating primary residency variable as originally described in the CES
residency <- residency %>% 
  mutate(
    case_id = as.character(caseid),
    residency_at_address = fct_collapse(residency, "Less than 1 year" = c("Less than 1 month", "2 to 6 months","7 to 11 months")),
         residency_at_address = fct_drop(residency_at_address)
         )
...
cc_full_raw <- common %>% 
  ...
  mutate(
    # this measure captures the length of time residing in the city where surveyed and is captured as integer values
    residency_city = case_when(
      # catches those who have logically valid values
      !is.na(citylength_1) & age >=citylength_1 & citylength_1 > 0 ~ citylength_1, # catches those who have valid values
      .default = NA
    ),
    # residency at city is missing for two key years 2012 and 2020. Can impute these with the residency at address which is logically contained within the residency in the same city. Only creating the binary here.
    residency_city_imputed_gt5 = case_when(
      !is.na(residency_city) & residency_city >= 5 ~ "5 or more years",
      is.na(residency_city) & residency_at_address == "5 or more years" ~ "5 or more years",
      is.na(residency_city) & is.na(residency_at_address) ~ NA,
      .default = "Less than 5 years"
    )
    )

cc_full_raw %>% count(year, residency_city, residency_city_imputed_gt5, residency_at_address ) %>% filter(year %in% c(2010, 2012, 2014, 2016, 2018, 2020)) %>% print(n = 10000)
```
...
demo_vars <-
  c("gender", "age", "educ","hispanic","race","race_h","citizen","religion",
    ...
    "residency_at_address", "residency_city", "residency_city_imputed_gt5"
  )
...
Current address is the ideal measure here given that some cities spread actross multiple counties. However if I only use residency at current address then I lose the 2018 year. Given the overlap with thes potential residency variables and 
...
Burkey, Mark L.  "Gini Coefficients for the 2000 Census", March 2006.  https://sites.google.com/a/burkeyacademy.com/main/home/gini-coefficients. These are not actually used in the final manuscript. 
```{r}
...
```{r}
fwd_res_at_addressgte5 <- fwd %>% filter(residency_at_address == "5 or more years")
fwd_res_citygte5 <- fwd %>% filter(residency_city >= 5)
fwd_res_city_imputedgte5 <- fwd %>% filter(residency_city_imputed_gt5 == "5 or more years")
saveRDS(fwd_res_at_addressgte5, "../data/processed/fwd_res_at_addressgte5.rds")
saveRDS(fwd_res_citygte5, "../data/processed/fwd_res_citygte5.rds")
saveRDS(fwd_res_city_imputedgte5, "../data/processed/fwd_res_city_imputedgte5.rds")
```
